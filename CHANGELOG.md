...
- **NVIDIA Control layout (Main tab)** - RTX HDR and Low latency controls use consistent widths: RTX HDR sliders (Contrast, Middle Grey, Peak Brightness, Saturation, RTX Dynamic Vibrance Saturation/Value) have a 300px label column and 500px slider width; Ultra Low Latency selectors are 400px wide and start at 500px (SameLine); Max pre-rendered frames slider is 1200px wide with label at 300px. Details: `main_new_tab.cpp` `nvidia_slider_label_width`, `nvidia_slider_width`, `low_latency_ull_label_width`, `low_latency_slider_width`, ULL combo width in `draw_combo_or_checkbox`.
