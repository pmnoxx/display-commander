...
- [removal] [settings] [ui] **Force Flip Discard upgrade removed** - Removed the FLIP_SEQUENTIAL to FLIP_DISCARD swapchain upgrade option (`force_flip_discard_upgrade`), its DXGI Control UI subsection, status tracking globals, and create-time upgrade logic. **Enable Flip Chain** (Advanced) still upgrades legacy flip models separately.
- [removal] [settings] [ui] **Max frame latency override removed** - Removed the runtime `SetMaximumFrameLatency` combo (No override / 1–16), its `max_frame_latency_override` setting, per-swapchain apply logic in `OnPresentUpdateBefore`, and the DXGI Control UI.
- [removal] [settings] [ui] **DXGI VSync override removed** - Removed the runtime DXGI Present SyncInterval override (No override / Force ON / 1/2–1/4 / FORCED OFF dropdown), its `vsync_override` setting, and Present-hook logic that forced sync interval and tearing flags. DXGI games now use the game's own VSync setting; **Prevent Tearing** is unchanged.
