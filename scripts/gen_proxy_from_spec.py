...
def default_return_type(name, dll_name):
    dll_lower = dll_name.lower().replace(".dll", "")
    if dll_lower in ("vulkan-1", "vulkan_1"):
        # Vulkan: void for Destroy/Free/Cmd* (and stubs); int (VkResult) for the rest
        if name.startswith("vkDestroy") or name.startswith("vkFree") or name.startswith("vkCmd"):
            return "void"
        if name in ("vkUnmapMemory", "vkUnmapMemory2", "vkSetEvent", "vkResetEvent", "vkTrimCommandPool"):
            return "void"
        return "int"  # VkResult or other 32-bit
...
def gen_proxy_cpp(entries, dll_name):
    dll_lower = dll_name.lower().replace(".dll", "")
    # C-safe identifier (e.g. vulkan-1 -> vulkan_1)
    var = dll_lower.replace(".", "_").replace("-", "_")
    lines = [
        "/*",
    ...
    if dll_lower == "winmm":
    ...
    elif dll_lower == "vulkan-1":
        # vulkan-1.dll: load from system directory (same pattern as d3d11, no fallback)
        load_fn = "LoadRealVulkan1"
        lines += [
            "",
        ...
        load_fn = "LoadReal" + "".join(x.capitalize() for x in dll_lower.replace("-", "_").split("_"))
        lines += [
            "",
    ...
    for name, is_stub, params, use_gdi32, ordinal in entries:
        ...
        if is_stub:
            ...
            lines.append("extern \"C\" %s WINAPI %s(%s) {" % (ret, name, param_decl))
            lines.append("    CALL_GUARD(utils::get_now_ns());")
            fail_ret = "return;" if ret == "void" else ("E_FAIL" if ret == "HRESULT" else "0")
            if dll_lower == "d3d11" and use_gdi32:
                lines.append("    if (!LoadRealD3D11() && !LoadRealGdi32()) %s" % ("return;" if ret == "void" else "return 0;"))
            else:
                lines.append("    if (!%s()) %s" % (load_fn, "return;" if ret == "void" else "return %s;" % fail_ret))
            lines.append("    typedef %s (WINAPI *PFN)(%s);" % (ret, param_list))
            lines.append("    PFN fn = (PFN)(%s);" % call_get)
            lines.append("    if (!fn) %s" % ("return;" if ret == "void" else "return %s;" % fail_ret))
            if ret == "void":
                lines.append("    fn(%s);" % ", ".join(param_names))
            else:
