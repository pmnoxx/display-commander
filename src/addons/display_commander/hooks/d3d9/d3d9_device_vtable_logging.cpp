...
// Vtable indices for IDirect3DDevice9 (from d3d9.h declaration order; 0-2 = IUnknown).
// Present = 17 (hooked in d3d9_present_hooks). Create* resource methods 21-28.
namespace VTable {
constexpr UINT CreateTexture = 21;
constexpr UINT CreateVolumeTexture = 22;
constexpr UINT CreateCubeTexture = 23;
constexpr UINT CreateVertexBuffer = 24;
constexpr UINT CreateIndexBuffer = 25;
constexpr UINT CreateRenderTarget = 26;
constexpr UINT CreateDepthStencilSurface = 27;
constexpr UINT CreateOffscreenPlainSurface = 28;
}  // namespace VTable
...
// Detours: RECORD_DETOUR_CALL, call original, log on error (same pattern as ReShade-wrapped device methods).
...
static HRESULT STDMETHODCALLTYPE CreateVolumeTexture_Detour(IDirect3DDevice9* This, UINT Width, UINT Height,
                                                            UINT Depth, UINT Levels, DWORD Usage, D3DFORMAT Format,
                                                            D3DPOOL Pool, IDirect3DVolumeTexture9** ppVolumeTexture,
                                                            HANDLE* pSharedHandle) {
    RECORD_DETOUR_CALL(utils::get_now_ns());
    HRESULT hr = CreateVolumeTexture_Original(This, Width, Height, Depth, Levels, Usage, Format, Pool,
...
static HRESULT STDMETHODCALLTYPE CreateCubeTexture_Detour(IDirect3DDevice9* This, UINT EdgeLength, UINT Levels,
                                                          ...
                                                          HANDLE* pSharedHandle) {
    RECORD_DETOUR_CALL(utils::get_now_ns());
    HRESULT hr = CreateCubeTexture_Original(This, EdgeLength, Levels, Usage, Format, Pool, ppCubeTexture, pSharedHandle);
    if (FAILED(hr)) {
...
// Detour: log call, call original, log on error
...
void InstallD3D9DeviceVtableLogging(IDirect3DDevice9* device) {
    ...
    bool ok = true;
    if (!CreateAndEnableHook(vtable[VTable::CreateTexture],
                             reinterpret_cast<LPVOID>(&CreateTexture_Detour),
                             reinterpret_cast<LPVOID*>(&CreateTexture_Original),
                             "IDirect3DDevice9::CreateTexture")) {
        LogWarn("InstallD3D9DeviceVtableLogging: CreateTexture hook failed");
        ok = false;
    }
    if (!CreateAndEnableHook(vtable[VTable::CreateVolumeTexture],
                             reinterpret_cast<LPVOID>(&CreateVolumeTexture_Detour),
    ...
    if (!CreateAndEnableHook(vtable[VTable::CreateCubeTexture],
                             reinterpret_cast<LPVOID>(&CreateCubeTexture_Detour),
    ...
    if (!CreateAndEnableHook(vtable[VTable::CreateVertexBuffer],
                             reinterpret_cast<LPVOID>(&CreateVertexBuffer_Detour),
    ...
    if (!CreateAndEnableHook(vtable[VTable::CreateIndexBuffer],
                             reinterpret_cast<LPVOID>(&CreateIndexBuffer_Detour),
    ...
    if (!CreateAndEnableHook(vtable[VTable::CreateRenderTarget],
                             reinterpret_cast<LPVOID>(&CreateRenderTarget_Detour),
    ...
    if (ok) {
        LogInfo("InstallD3D9DeviceVtableLogging: device vtable logging installed (CreateTexture, CreateVolumeTexture, CreateCubeTexture, CreateVertexBuffer, CreateIndexBuffer, CreateRenderTarget, CreateDepthStencilSurface, CreateOffscreenPlainSurface)");
    }
}
