                ...
                if (count > 0) {
                    ...
                    if (count < RECENT_SAMPLES_SIZE) {
                        ...
                        for (size_t i = 0; i < RECENT_SAMPLES_SIZE; ++i) {
                            ...
                            if (m_recent_samples[idx].refresh_rate < min_val)
                                min_val = m_recent_samples[idx].refresh_rate;
                            if (m_recent_samples[idx].refresh_rate > max_val)
                                max_val = m_recent_samples[idx].refresh_rate;
                        }
                    }
