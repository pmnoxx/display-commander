...
typedef HRESULT(WINAPI* PFN_D3D12CreateVersionedRootSignatureDeserializer)(LPCVOID pSrcData, SIZE_T SrcDataSizeInBytes,
                                                                           REFIID pRootSignatureDeserializerInterface,
                                                                           void** ppRootSignatureDeserializer);
typedef HRESULT(WINAPI* PFN_D3D12EnableExperimentalFeatures)(UINT NumFeatures, const IID* pIIDs,
...
static bool LoadRealD3D12() {
    if (g_d3d12_module != nullptr) return true;
...
extern "C" HRESULT WINAPI D3D12CreateDevice(IUnknown* pAdapter, D3D_FEATURE_LEVEL MinimumFeatureLevel, REFIID riid,
                                            void** ppDevice) {
...
extern "C" HRESULT WINAPI D3D12CreateRootSignatureDeserializer(LPCVOID pSrcData, SIZE_T SrcDataSizeInBytes,
                                                               REFIID pRootSignatureDeserializerInterface,
                                                               void** ppRootSignatureDeserializer) {
    if (!LoadRealD3D12()) return E_FAIL;

    auto func = reinterpret_cast<PFN_D3D12CreateRootSignatureDeserializer>(
        GetProcAddress(g_d3d12_module, "D3D12CreateRootSignatureDeserializer"));
    if (func == nullptr) return E_FAIL;

    return func(pSrcData, SrcDataSizeInBytes, pRootSignatureDeserializerInterface, ppRootSignatureDeserializer);
}

extern "C" HRESULT WINAPI D3D12CreateVersionedRootSignatureDeserializer(LPCVOID pSrcData, SIZE_T SrcDataSizeInBytes,
                                                                        REFIID pRootSignatureDeserializerInterface,
                                                                        void** ppRootSignatureDeserializer) {
    if (!LoadRealD3D12()) return E_FAIL;

    auto func = reinterpret_cast<PFN_D3D12CreateVersionedRootSignatureDeserializer>(
        GetProcAddress(g_d3d12_module, "D3D12CreateVersionedRootSignatureDeserializer"));
    if (func == nullptr) return E_FAIL;

    return func(pSrcData, SrcDataSizeInBytes, pRootSignatureDeserializerInterface, ppRootSignatureDeserializer);
}

extern "C" HRESULT WINAPI D3D12EnableExperimentalFeatures(UINT NumFeatures, const IID* pIIDs,
                                                          void* pConfigurationStructs,
                                                          UINT* pConfigurationStructSizes) {
    if (!LoadRealD3D12()) return E_FAIL;

    auto func = reinterpret_cast<PFN_D3D12EnableExperimentalFeatures>(
        GetProcAddress(g_d3d12_module, "D3D12EnableExperimentalFeatures"));
    if (func == nullptr) return E_FAIL;
...
extern "C" HRESULT WINAPI D3D12SerializeRootSignature(const D3D12_ROOT_SIGNATURE_DESC* pRootSignature,
                                                      D3D_ROOT_SIGNATURE_VERSION Version, ID3DBlob** ppBlob,
                                                      ID3DBlob** ppErrorBlob) {
    if (!LoadRealD3D12()) return E_FAIL;

    auto func = reinterpret_cast<PFN_D3D12SerializeRootSignature>(
        GetProcAddress(g_d3d12_module, "D3D12SerializeRootSignature"));
    if (func == nullptr) return E_FAIL;
...
extern "C" HRESULT WINAPI D3D12SerializeVersionedRootSignature(
    ...
    auto func = reinterpret_cast<PFN_D3D12SerializeVersionedRootSignature>(
        GetProcAddress(g_d3d12_module, "D3D12SerializeVersionedRootSignature"));
    if (func == nullptr) return E_FAIL;
