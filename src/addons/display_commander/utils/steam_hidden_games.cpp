...
#include "steam_hidden_games.hpp"
#include "logging.hpp"
...
constexpr const wchar_t* kBaseKey = L"Software\\Display Commander\\SteamHiddenGames";
...
void AddSteamGameToHidden(uint32_t app_id) {
    ...
    if (st != ERROR_SUCCESS || !hKey) {
        LogInfo("Steam hidden games: failed to open key, error %ld", (long)st);
        return;
    }
    ...
    RegCloseKey(hKey);
    if (st != ERROR_SUCCESS) {
        LogInfo("Steam hidden games: failed to write app_id %u, error %ld", app_id, (long)st);
    }
}
